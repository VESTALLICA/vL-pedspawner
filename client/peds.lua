...
local function spawnPeds()
    ...
    for k, v in pairs(Config.Peds) do
        ...
        local a = math.random(1, #v["coords"])  
        local model = v["coords"][a]
        ...
        SetPedToInformRespectedFriends(CreatedPeds[k], 100.0, 20) -- might inform friendlies on target loss or target contact. 
        ...
        -- combat --
        SetPedCombatMovement(StolkovGuards[k], v["movement"])
        ...
        SetPedPathCanUseLadders(CreatedPeds[k], ["ladders"]) -- can use ladders 
        SetPedPathCanUseClimbovers(CreatedPeds[k], ["climb"]) -- can climb
        SetPedPathCanDropFromHeight(CreatedPeds[k], ["drop"]) -- can drop from height
        SetEntityMaxSpeed(CreatedPeds[k], v["movespeed"])

        
        -- WEAPONS
        if v["primary"] then
            GiveWeaponToPed(CreatedPeds[k], 'weapon_'..v["primary"]..'', ["primaryammo"], true, true) -- give gun with 999 ammo. weapon_ is predefined for qol
        end
        if v["secondary"] then
            GiveWeaponToPed(CreatedPeds[k], 'weapon_'..v["primary"]..'', ["primaryammo"], true, true) -- give gun with 999 ammo. weapon_ is predefined for qol
